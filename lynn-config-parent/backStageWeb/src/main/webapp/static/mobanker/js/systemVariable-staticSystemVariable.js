...
function InitAModule() {
	...
	if (aSystemName != "") {
		$.ajax({
			url : "../systemModule/allSonModuleName/" + aSystemName + "/STATIC",
			type : "get",
			dataType : "json",
...
function InitModule(moduleName, operation) {
	...
	if (systemName != "") {
		$.ajax({
			url : "../systemModule/allSonModuleName/" + systemName + "/STATIC",
			type : "get",
			dataType : "json",
			success : function(result) {
				if (result == null || result.status != 1 || result.status == null || result.data.resultStatus != 1) {
...
function save() {
	...
	var url = "addOrUpdateSystemVariable/static";
	var id = $('#aId');
	...
	$.post(url, {
		...
		if (data.data.resultStatus == "1") {
			bootbox.alert(data.msg + "<br/><font color='red'>静态变量必须重启服务器方可生效<font>", function() {
				//initData("-");
...
function updateStatus(id, status) {
	...
	bootbox.confirm("您确定要" + flag + "这个变量吗?", function(data) {
		if (data) {
			...
			$.post("addOrUpdateSystemVariable/static", {
				'id' : id,
				'status' : status,
			}, function(data) {
				if (data.data.resultStatus == "1") {
